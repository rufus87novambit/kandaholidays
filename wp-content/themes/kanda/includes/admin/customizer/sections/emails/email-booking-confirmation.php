...
return array(
    ...
    'fields' => array(
        'email_booking_confirmation_title' => array(
            ...
            'description'   => esc_html__( 'Variables: {{SITE_NAME}} -> Link to home page, {{BOOKING_NUMBER}} -> Booking number, {{AGENCY_NAME}} -> Booked agency name, {{HOTEL_NAME}} -> Hotel name, {{ROOM_TYPE}} -> Room type, {{CHECK_IN}} -> Check in date, {{CHECK_OUT}} -> Check out date, {{PASSENGERS}} -> Passenger names, {{CANCELLATION_DETAILS}} -> Cancellation details', 'kanda' ),
            'section'  => $section_id,
