...
class IOL_Request {
    ...
    public function search_by_hash( $hash ) {
        $request_args = Kanda_Request_Cache::get_request_by_hash( $hash );
        if( $request_args ) {
     ...
     * @throws Exception
     */
    public function search_hotels( $request_args = array() ) {

        $cache = Kanda_Request_Cache::get_by( 'request', $request_args, false, '`response`' );
        if( $cache ) {
            $cache = json_decode( stripslashes( $cache->response ) );

            $response = new Kanda_Response( $cache->code, $cache->data, $cache->message );
        } else {
            $xml = $this->generate_xml( 'hotel-search-request', $request_args );

            $response = $this->process( $xml, $request_args );

            if( $response->valid() ) {
                Kanda_Request_Cache::cache( $response, $request_args, 'iol' );
            }
        }
